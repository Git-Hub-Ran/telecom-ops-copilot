...
Uses the azure-ai-agents package (the dedicated agents SDK), which provides
the clean agents_client.files / agents_client.vector_stores / agents_client.threads
API surface we need. The companion azure-ai-projects package exists but has a
different API surface (the .agents property returns AgentsOperations without a
.files namespace, which is why an earlier attempt failed).
"""
